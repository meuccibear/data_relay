...
import com.zp.data_relay.exception.MsgException;
...
//        }
    ...
    public static long toLon(String value, Long defaultValue) {
        if (ObjectUtils.notIsEmpty(value)) {
            return toLon(value);
        }
        return defaultValue;
    ...
    public static long toLon(String value) {
        if (valueVerification("isInteger", value)) {
...
//                        System.out.println(String.formatKV("[for]%d", surplus));
            ...
            int curNum = 0;
            for (int i = 0; i < n; i++) {
                List<?> objects = data.subList(curNum, curNum + itemSSize.get(i));
                if(ObjectUtils.notIsEmpty(objects)){
                    datas.add(objects);
                    curNum += itemSSize.get(i);
                }
            }
